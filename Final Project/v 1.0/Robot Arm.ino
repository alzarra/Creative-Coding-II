// Not my orginal work I copy some and modifed a lot 
// library
...
{
  ist[0] = map(SensVal[0], 150, 900, 600, 2400);//  rotate
...
void play_servo()
{
    ...
    if (steps < stepsMax) // sure we not reach the end from a move
    {
      ...
      servo_0.writeMicroseconds(ist[3]); // gribber// mapped connections 
...
void Button() // check buttons for single and doubleclick
{
  ...
  {
    delay(20);
    if (digitalRead(6) == true) // taster relased
    {
      if (Taster == 0)
      {
        Taster = 1;
